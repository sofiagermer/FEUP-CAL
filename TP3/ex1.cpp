...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for (int i = 0; i < vp.size(); i++) {
        for (int j = i+1; j < vp.size(); ++j) {
            if (vp.at(i).distance(vp.at(j)) <= min) {
                min = vp.at(i).distance(vp.at(j));
                res = Result(min, vp.at(i), vp.at(j));
            }
        }
    }
...
Result rec_x(vector<Point> &P){
    if(P.size() == 2) return Result(P.at(0).distance(P.at(1)), P.at(0), P.at(1));
    if(P.size() == 1)return Result(MAX_DOUBLE, P.at(0), P.at(0));
    double mid_x = (P.at(0).x + P.at(P.size()-1).x)/2;
    vector <Point> right;
    vector <Point> left;
    for(auto &p :P){
        if(p.x <= mid_x) left.push_back(p);
        else right.push_back(p);
    }
    Result l_res = rec_x(left);
    Result r_res = rec_x(right);
    if(l_res.dmin < r_res.dmin) return l_res;
    else return r_res;
}
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    ...
    double delta = rec_x(vp).dmin;
    double mid_x = (vp.at(0).x + vp.at(vp.size()-1).x)/2;
    double left_stripe = mid_x - delta;
    double right_stripe = mid_x + delta;
    for(int i = 0; i < vp.size(); i++){
        if(vp.at(i).x < left_stripe) vp.erase(vp.begin()+i);
        else if(vp.at(i).x > right_stripe) vp.erase(vp.begin()+i);
    }
    sortByY(vp, 0 , vp.size()-1);
    int temp_d = MAX_DOUBLE;
    for(int i = 0; i < vp.size() -1; i++){
        for(int j= 0; i < vp.size(); j++ ){
            temp_d = vp.at(i).distance(vp.at(j));
            if(temp_d < delta){
                res = Result(temp_d, vp.at(i), vp.at(j));
                delta = temp_d;
            }
        }
    }
    ...
    //SORT BY Y -> acho que só é preciso dar sort pelo y depois de recursivamente ter feito pelo x e o delta que vamos usar é a distancia minima em termos de x que encontrarmos
    /*sortByY(vp,0,vp.size()-1);
    for(auto &q : vp){
        Q.push_back(q);
    }*/
    ...
    if(vp.size() == 2){
        ...
        return Result(MAX_DOUBLE, vp.at(0),vp.at(0));
    }
    else{
        middle =  vp.at
        if( nearestPoints_DC(left).dmin < nearestPoints_DC(right).dmin) return nearestPoints_DC(left);
        else return nearestPoints_DC(right);
    }

    for(int i = 0; i < numPointsInStrip; i++){
        for(i +1; j < numPointsInStrip; j++){
            if
        }
    }*/
}
