...
private:
    ...
    void clear();
	//TODO: Add other methods if needed
    int choose_j_to_fill(int i);
    void choose_cell_to_fill(int (&chosen_cell)[2]);
    bool Rec(int old_counter, int counter, int lastNumber);
};

// Ex 3
bool changeMakingBacktracking(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]);
bool changeRec(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]);
// Ex 4
class Activity {
public:
    unsigned int start;
...
bool Rec(std::vector<Activity> A, std::vector< Activity> &final);
